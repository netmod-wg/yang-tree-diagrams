...
in a module schema definition and can identified in a tree diagrams
...
provide via the Schema Mount module defined in
...
It is worth highlighting that the OSPF module is an augmentation to the
Routing module, but as it is listed in the Schema Mount module (or inline
YANG library) its top level nodes are included in the tree diagram.

A mount point definition alone is not sufficient to identify if a
...
associated with the specific mount point and is indicated on the top
level mounted nodes. For example in the above tree, when the "config"
for the routing module indicates false, the only change would be to the
